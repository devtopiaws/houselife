...
def sincronizar_inventario(request):
    productos_actualizados = []  # Inicializar al principio para evitar UnboundLocalError
    productos_con_stock_django = {}  # Inicializar el diccionario vacío

    try:
        # Obtener los productos desde WooCommerce
        productos_woocommerce = obtener_productos()
        # print(f"Productos obtenidos: {productos_woocommerce}")  # Para verificar que llegan productos

        # Verificar si hay productos obtenidos desde WooCommerce
        if productos_woocommerce:
            for producto_wc in productos_woocommerce:
                ...
                # Buscar el producto en Django (coincidencia exacta, ignorando mayúsculas)
                producto_django = Product.objects.filter(name__iexact=nombre_producto_wc).first()

                if producto_django:
                    # Guardar el stock de Django en el diccionario
                    productos_con_stock_django[producto_wc['name']] = producto_django.stock

                    # Si el stock es diferente, actualizar el producto en Django
            ...
            productos_woocommerce = []  # Asegurar que sea una lista vacía si no hay productos

    except Exception as e:
        print(f"Error al sincronizar el inventario: {str(e)}")
        productos_woocommerce = []  # En caso de error, mantener una lista vacía

    # Paginación de los productos (10 productos por página)
    paginator = Paginator(productos_woocommerce, 10)
    page_number = request.GET.get('page', 1)  # Obtener el número de página de la URL
    page_obj = paginator.get_page(page_number)  # Obtener los productos para la página solicitada

    # Renderizar la plantilla con los datos necesarios
    return render(request, 'woocommerce/sincronizar_inventario.html', {
        'page_obj': page_obj,
        'productos_actualizados': productos_actualizados,
        'productos_con_stock_django': productos_con_stock_django,
    })
